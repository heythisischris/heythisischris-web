...
export const Project = () => {
    ...
    return <div className='flex flex-col gap-2 w-full p-4 pt-0'>
        <div className="flex flex-col gap-4">
            <Link onTouchStart={() => navigate('/portfolio')} className='shadow-[2px_2px_0_1px] shadow-border border-text border-[1px] rounded-md w-fit px-2 text-text' to='/portfolio'>← Go back</Link>
            ...
            <motion.div initial={{ opacity: 0, y: -10 }}
                ...
                {project?.links?.map(({ title, link }, index) => <ul key={index} className='shadow-[2px_2px_0_1px] shadow-border border-text border-[1px] rounded-md px-2 py-1'>
                    <b>{title}</b>
                    <li>
